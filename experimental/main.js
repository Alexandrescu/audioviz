...
function load() {
    ...
    for ( var j = 0; j < sizeY; j++) {
        for ( var i = 0; i < sizeX; i++) {
            ...
            // noisePixels.push(0.5, 0.5, 0.5, 1);
            pixels.push(0, 0, 0, 1);
            ...
            var angleRand = Math.random() * Math.PI;
            var radius = (Math.random() - 0.5) / 3 ;
            var xRand = radius * Math.cos(angleRand) + 0.5;
            var yRand = radius * Math.sin(angleRand) + 0.5;
            if (i < particlesWidth && j < particlesHeight) {
                 particlesIdx.push(center.x + xRand, center.y + yRand);
            }
                    
            // particles.push(center.x + xRand, center.y + yRand, 0, 0);
    ...
    for ( var i = 0; i < particlesIdx.length; i += 2) {
        particles.push(particlesIdx[i], particlesIdx[i+1], 0, 0); // initial particle positions, here: uniform distribution
    }
...
function anim() {
    ...
    rainbowR = 0.2 + 0.5 * Math.sin(t);
    rainbowG = 0.2 + 0.5 * Math.sin(t + w);
    rainbowB = 0.2 + 0.5 * Math.sin(t - w);
